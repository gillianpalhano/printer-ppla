 ...
 */
export function sendGraphic(this: Printer, { imageBuffer, name, flipped = true, memory = this.config.defaultSaveGraphic }: IsendGraphic): void {
 ...
 */
export function addGraphic(this: Printer, { y, x, name, deleteAfter = true, memory = this.config.defaultSaveGraphic }: IGraphic): void { // 1Y11000yyyyxxxxn..n
  if(name.length > 16) throw new Error('16 character name limit exceeded')
 ...
 * @param memory - Memory location where it was saved
 */
export function deleteGraphic(this: Printer, { name, memory = this.config.defaultSaveGraphic }: IDeleteGraphic): void { // 1Y11000yyyyxxxxn..n
  if(name.length > 16) throw new Error('16 character name limit exceeded')
